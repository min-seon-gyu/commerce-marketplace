...
| **AI 프로모션 자동화** | 자연어 입력 → 프로모션 초안 생성 + 결정적 서버사이드 가드레일 검증. AI는 제안, 서버가 결정. 킬스위치로 키 없이 부팅. | `PromotionDraftService` (Claude API) |
...
Aggregate 중심 모듈러 모놀리스. 10개 모듈, 130개 소스 파일, 약 42개 API 엔드포인트. 각 모듈은 `domain / application / infrastructure / interfaces` 4계층(헥사고날)으로 분리된다.
...
├── promotion/    ← 쿠폰/프로모션 (Promotion + Coupon + 결합결제 오케스트레이터 + AI 초안)
...
### 8. AI 프로모션 어시스턴트 (AI Promotion Assistant)

운영자가 자연어로 캠페인 의도를 입력하면 구조화된 프로모션 초안을 생성한다. **AI는 제안만, 서버가 결정한다.**

- **자연어 → 구조화 초안**: Claude(`claude-haiku-4-5`)에 `json_schema` 강제 출력으로 `PromotionDraft` 생성
- **결정적 가드레일** (`PromotionDraftValidator`): 허용 지역, 예산 상한, 할인율 범위 등 서버사이드 규칙으로 검증. 검증 통과 여부와 무관하게 초안 + `ValidationReport`를 반환하고, DB에는 쓰지 않음(사람이 검토 후 확정)
- **킬스위치(kill-switch)**: `ai.promotion.enabled` 기본 `false`. 비활성 시 `DisabledLlmClient`가 주입되어 **API 키 없이 부팅**(CI/로컬 기본값). 활성화는 `AI_PROMOTION_ENABLED=true` + `ANTHROPIC_API_KEY`
- **서킷 브레이커**: 연속 실패 임계 초과 시 회로 개방으로 외부 호출 차단(`SimpleCircuitBreaker`), 타임아웃/재시도/백오프 내장
- **골든셋 테스트**: 응답 파서를 고정 픽스처로 검증(라이브 콜 0) — 테스트는 외부 API를 호출하지 않음

### 9. 잔액 환불 (Balance Refund)
...
### 11. 거래 취소 — 보상 트랜잭션 (Compensating Transaction)
...
### 14. 복식부기 원장 및 정합성 검증 (Ledger & Verification)
...
### 15. 멱등키 처리 (Idempotency)

네트워크 불안정으로 인한 중복 요청을 안전하게 처리하는 체계.

- **대상 API**: 구매, 결제, 환불, 청약철회, 거래 취소, 쿠폰 발급, AI 초안 (금전/과금 관련 전체)
...
### 7. 왜 AI는 제안만 하고 서버가 결정하는가 (킬스위치)

LLM 출력은 비결정적이며 환각/한도 위반 위험이 있다. AI는 `json_schema`로 구조화 초안만 생성하고, **결정적 서버사이드 가드레일**이 허용 지역·예산·할인율을 검증한 뒤에야 사람이 확정한다. `ai.promotion.enabled` 기본 `false`(킬스위치)로 **API 키 없이 부팅**되며, 외부 장애는 서킷 브레이커로 격리한다.

### 8. 왜 멱등키 이중 저장인가

Redis로 빠르게 중복을 감지하고, DB에도 저장하여 Redis 장애 후에도 중복을 방지한다. 중복 감지 시 409가 아닌 **원래 응답을 원래 상태코드와 함께 반환**하여 클라이언트가 정상 흐름을 이어갈 수 있도록 한다.

### 9. 왜 원장 기록은 이벤트가 아닌 동기 호출인가
...
| | POST | `/api/v1/vouchers/purchase` | 구매 (멱등) |
| | POST | `/api/v1/vouchers/{id}/redeem` | 결제 — 쿠폰 결합 가능 (멱등) |
...
| | POST | `/api/v1/promotions/draft` | AI 프로모션 초안 (인증, 멱등) |
...
| AI | Claude API (`claude-haiku-4-5`, json_schema) — 킬스위치/서킷브레이커 |
...
- **AI는 기본 OFF**(킬스위치): API 키 없이도 부팅된다. AI 초안 기능을 켜려면 `AI_PROMOTION_ENABLED=true` + `ANTHROPIC_API_KEY`를 설정한다.
- **외부화된 환경변수**: `DB_HOST/DB_PORT/DB_NAME/DB_USERNAME/DB_PASSWORD`, `REDIS_HOST/REDIS_PORT`, `JWT_SECRET`, `AI_PROMOTION_ENABLED`, `ANTHROPIC_API_KEY`.
...
| **AI** | 응답 파서 골든셋(라이브 콜 0), 서킷 브레이커, 키 없이 부팅, 가드레일 검증 |
...
│   └── 04-ai-promotion-assistant.md   # AI 프로모션 어시스턴트 (스키마, 가드레일, 킬스위치)
├── src/main/kotlin/com/commerce/      # 130 소스 파일
...
│   ├── promotion/   ← Promotion + Coupon + RedemptionOrchestrator + AI 초안
...
├── src/test/kotlin/                   # 41 테스트 파일, 165 테스트 케이스

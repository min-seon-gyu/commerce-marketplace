...
│   │                                    CouponRedemption, DiscountType: FIXED/PERCENTAGE, PromotionDraft)
│   ├── application/
...
│   │   ├── PromotionDraftService / PromotionDraftValidator (AI 초안 + 결정적 검증)
│   │   └── RegionPolicy                 (가드레일: 허용 지역·예산 한도)
│   ├── infrastructure/
│   │   ├── PromotionBudgetManager.kt    (Redis 원자 예산 카운터, Lua reserve/release)
│   │   ├── Promotion/Coupon/CouponRedemption JpaRepository
│   │   └── ai/                          (LlmClient, ClaudeLlmClient, DisabledLlmClient,
│   │                                     SimpleCircuitBreaker, AiPromotionConfig/Properties)
│   └── interfaces/                     (PromotionController, MemberCouponController, PromotionDraftController)
│
├── point/                           ← 포인트 모듈 (신규)
...
| `POST /api/v1/promotions/draft` | AI 초안 생성. 동일 요청 재시도 시 토큰/비용 중복 소모 방지 |
...
### AI 게이팅 (프로모션 어시스턴트)
- **킬스위치** `ai.promotion.enabled`(기본 `false`): false면 `DisabledLlmClient` 주입 → **API 키 없이 부팅**(CI). true일 때만 `ClaudeLlmClient` 구성(`@ConditionalOnProperty`).
- 운영 가드: 토큰/비용 상한(`maxTokens`), 연결/읽기 타임아웃, 지수 백오프 재시도, `SimpleCircuitBreaker`(연속 실패 임계치 초과 시 오픈).
- **결정적 가드레일**(허용 지역·예산 상한·날짜 유효성·스택 금지)이 AI 출력을 서버측 검증하며, AI는 제안만 하고 DB에 직접 쓰지 않는다.
- 상세 설계는 [`04-ai-promotion-assistant.md`](04-ai-promotion-assistant.md) 참조(여기서 중복 기술하지 않음).
...
| `ai.promotion.draft.latency` / `.tokens` / `.failure`(reason) / `.count`(result) | AI 초안 지연·토큰·실패원인·가드레일 통과/거부 |

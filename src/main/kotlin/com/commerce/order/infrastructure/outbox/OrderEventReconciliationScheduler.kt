 ...
 * 이벤트 로그가 누락된 "published-but-not-applied" 행이 생길 수 있다. 이를 주기적으로 anti-join으로 찾아 멱등 재적용한다.
 * (Kafka 활성 시에만 동작 — Direct 경로는 자체 attempts/격리로 자기완결적.)
 */
@Component
@ConditionalOnProperty(prefix = "order.kafka", name = ["enabled"], havingValue = "true")
class OrderEventReconciliationScheduler(
    ...
    @Value("\${order.outbox.reconcile-grace-ms:600000}") private val graceMs: Long,
) {
    private val log = LoggerFactory.getLogger(javaClass)

    @Scheduled(fixedDelayString = "\${order.outbox.reconcile-ms:300000}")
    fun reconcile() {
        reconcileOnce()
    }
    ...
    fun reconcileOnce(): Int {
        ...
        outboxRepository.findUnappliedPublished(maxAttempts, threshold, PageRequest.of(0, 200)).forEach { event ->
            try {
                ...
                recovered++
                log.info("Reconciled missing order event log for outbox {} ({})", event.id, event.eventType)
            } catch (e: Exception) {

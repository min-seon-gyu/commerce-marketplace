...
interface OutboxEventRepository : JpaRepository<OutboxEvent, Long> {
    ...
    fun findTop200ByPublishedFalseAndAttemptsLessThanOrderByIdAsc(maxAttempts: Int): List<OutboxEvent>

    /**
     * Reconciliation: 발행 완료(Kafka send)됐으나 감사 로그가 누락된 행(anti-join)을 직접 찾는다.
    ...
    @Query(
        ...
        where o.published = true and o.attempts < :maxAttempts and o.createdAt < :threshold
          and not exists (select a.id from AuditLog a where a.eventId = o.eventId)
        order by o.id asc
        """
    )

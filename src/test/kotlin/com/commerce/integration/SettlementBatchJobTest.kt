...
/**
 * 결산 주기 일괄 정산 배치 E2E. APPROVED 판매자 전체를 청크 처리해 지역 정산주기(기본 MONTHLY) 구간의
 * 정산을 생성하고, 검증 스텝까지 COMPLETED 되는지 확인한다. 0원 스킵과 재실행 멱등도 함께 검증한다.
 *
 * DB가 테스트 간 공유되므로(정리 없음) 배치는 타 테스트의 판매자까지 처리한다 — 단언은 이 테스트가 만든
 * 판매자(sellerId)으로만 스코프한다.
 */
@SpringBatchTest
class SettlementBatchJobTest : IntegrationTestSupport() {
    ...
    fun `batch creates settlements for approved sellers with sales and skips zero, idempotent on rerun`() {
        ...
        val salesSettlements = settlementRepository.findAll().filter { it.sellerId == sellerWithSales.id }
        ...
        // 0원 판매자은 정산 없음
        settlementRepository.findAll().none { it.sellerId == sellerNoSales.id }.shouldBeTrue()
        ...
        settlementRepository.findAll().count { it.sellerId == sellerWithSales.id } shouldBe 1
    }
}

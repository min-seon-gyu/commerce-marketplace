 ...
 * **무성 유실되지 않음**을 검증한다(at-least-once 유실 방지).
 */
@EmbeddedKafka(
    partitions = 1,
...
class OrderEventKafkaDltTest : IntegrationTestSupport() {
    ...
    fun `consumer failure routes the message to the dead letter topic instead of dropping it`() {
        ...
        kafkaTemplate.send("order-events", "k", "this-is-not-json").get()
        ...
        consumer.use {
            it.subscribe(listOf("order-events.DLT"))
            // 백오프 재시도 소진 후 DLT 발행 → 최대 20초 대기
            val record = KafkaTestUtils.getSingleRecord(it, "order-events.DLT", Duration.ofSeconds(20))

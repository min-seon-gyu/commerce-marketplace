...
class ClaudeResponseParserGoldenSetTest {
    ...
    // Note: BigDecimal shouldBe is scale-sensitive; fixtures deliberately use whole numbers
    // (e.g. BigDecimal("10") not BigDecimal("10.0")) to match Jackson IntNode.decimalValue() scale=0.
    companion object {
        data class Golden(
            val intent: String,
            ...
            override fun toString() = intent // used as @ParameterizedTest display name
        }

        @JvmStatic
        fun goldenCases(): List<Arguments> = listOf(
            Golden(
    ...
    fun `골든셋 자연어 의도에서 구조화 규칙 파싱 회귀`(g: Golden) {
    ...
    @ParameterizedTest(name = "[{index}] {0}")
    @MethodSource("goldenCases")
    fun `골든셋 파싱된 규칙의 가드레일 검증 결과 회귀`(g: Golden) {
        val parsed = parser.parse(claudeBody(g.draftJson))
    ...
    fun `토큰 사용량을 합산해 노출한다`() {
        val draftJson = """{"name":"성남시 7월 10% 할인","discountType":"PERCENTAGE","discountValue":10,"target":"SN","budgetCap":50000000,"minSpend":10000,"validFrom":"2026-07-01","validUntil":"2026-07-31","stackable":false}"""
        val parsed = parser.parse(claudeBody(draftJson, inputTokens = 800, outputTokens = 120))
    ...
    @Test
    fun `stackable 이 불리언이 아닌 문자열이면 결정적 에러로 변환된다`() {
        val bad = claudeBody("""{"name":"x","discountType":"PERCENTAGE","discountValue":10,"target":"SN","budgetCap":1,"minSpend":0,"validFrom":"2026-07-01","validUntil":"2026-07-31","stackable":"yes"}""")
        val ex = shouldThrow<BusinessException> { parser.parse(bad) }
        ex.errorCode shouldBe ErrorCode.AI_DRAFT_GENERATION_FAILED
    }

    @Test
    fun `usage 블록이 없는 응답은 결정적 에러로 변환된다`() {
        val noUsage = """
        {
          "id": "msg_test",
          "type": "message",
          ...
          "model": "claude-haiku-4-5",
          "stop_reason": "end_turn",
          "content": [{"type": "text", "text": ${mapper.writeValueAsString("""{"name":"x","discountType":"PERCENTAGE","discountValue":10,"target":"SN","budgetCap":1,"minSpend":0,"validFrom":"2026-07-01","validUntil":"2026-07-31","stackable":false}""")}}]
        }
        """.trimIndent()
        val ex = shouldThrow<BusinessException> { parser.parse(noUsage) }
        ex.errorCode shouldBe ErrorCode.AI_DRAFT_GENERATION_FAILED
    }
}
